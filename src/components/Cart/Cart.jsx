...
const Cart = ({cart, handleUpdateCartQty, handleRemoveCart, handleEmptyCart}) => {
    ...
    const EmptyCart = () => (
        <Typography variant='subtitle1'>No items? D:
         <Link to="/" className={classes.link}> Add some!</Link></Typography>
    );
        ...
        <Grid container spacing={3}>
            {cart.line_items.map((item) => (
                <Grid item xs={12} sm={4} key={item.id}>
                    <CardItem item={item} onUpdateCartQty={handleUpdateCartQty}
                    onRemoveFromCart={handleRemoveCart}></CardItem>
                </Grid>
            ))}
        </Grid>
...
<Container>
    ...
    <Typography className={classes.title} variant="h3" gutterBottom style={{paddingTop: '5%'}}>
    Your shopping Cart
    </Typography>
